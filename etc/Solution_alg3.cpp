    			...
    			if(!(wm == 1 && obj_bnd < 0)){
    				...
    				if(subsets.size() > 1){
    					...
    					for(auto set: subsets){
    						...
    						if(!found_base){//caso o subset não tenha a base insere no vetor
    							subsets_S.emplace_back(set);
    						}
    						else{//armazenar o subset que contém a base
...
//realizado para calcular o custo inicial de todos os grupos.
void Solution::initSol(solution* s){
	...
	for(int i=0;i<getNumberOfSets();++i){
		...
		if(p.pCost < 0){
			cout << "Error built initial solution!" <<endl;
			exit (EXIT_FAILURE);
		}

		//obter o melhor caminho para o grupo k
		paths.emplace_back(p);


		globalDepots.insert(paths.back().depots.begin(),paths.back().depots.end());

		//somar o custo de cada grupo
		cost += paths.back().pCost;

		//obter o maior custo e o índice da rota
	...
	//o vetor depots não possui a base, pois não iremos fazer shif ou swap da base
	solTemp.depots.insert(globalDepots.begin(),globalDepots.end());
	//considerar a base como depot
...
Solution::path Solution::MILP_Warm_Start(Set nodes_set,path initial_sol){
	...
	for(int d = 0; d < D-1; d++){//apenas depots sem a base
	...
	//Constraint 9
	GRBLinExpr rest9 = 0;
	for(int i=0; i < N; i++){// cada target
		//skip self_loop
		rest9 += vars_x[i][i];
	}
	model.addConstr(rest9 == 0, "Rest8");
	...
	//initial solution
		...
		for(auto edge:initial_sol.edges){
			i = map_cvset_id_to_node_id.find(edge.node_a)->second;
			j = map_cvset_id_to_node_id.find(edge.node_b)->second;

			vars_x_temp[i][j] = 1;

			if(j > D){
				fuel = input.getRobotFuel(robotID) - initial_sol.fuelOnTarget.find(edge.node_b)->second;
				vars_z[i][j].set(GRB_DoubleAttr_VarHintVal,fuel);
			}
		}
		for(i =0; i<N;i++)
			for(j =0; j<N;j++)
				vars_x[i][j].set(GRB_DoubleAttr_Start,vars_x_temp[i][j]);
	...
	for(int g2Index:graphsIndex){
		...
		for(int g1LID:g1LinesIndex){
			...
			for(int g2LID:g2LinesIndex){
				...
				if(isDefinitelyGreaterThan(solTemp.maxCost,maxCost, 1.0)){
					found_best_sol = true;
				}
			}
		}
	...
	if(!predictions_sol.empty()){
		...
		for(int i=0; i < n_of_prediction;i++){
			if(itMap !=  predictions_sol.end()){
				...
				// se o caminho em g1 for inviável para a inserção da linha do grupo g2
				if(pathG1.pCost < 0){
					continue;
				}

				pathG2 = itMap->second.p2;
				// se o caminho em g2 for inviável para a inserção da linha do grupo g1
				if(pathG2.pCost < 0){
					continue;
				}
				...
				if(isDefinitelyGreaterThan(max(s->paths[g1].pCost,s->paths[g2].pCost),
					...
					//insere na fronte a solução temporária, que é melhor apenas na quantidade de depots
...
bool Solution::shift(solution* s){
	...
	while(!vLines.empty()){//enquanto não testar todas as linhas
		...
		for(int g2Index : graphsIndex){
			...
			if(found_best_sol)
				break;
			if(g1Index!=g2Index){//com exceção de g1Index
...
bool Solution::improveSol(solution* s){
	...
	for(uint i = 0; i < s->paths.size();++i){
		...
		//caso seja a solução seja inviável
		if(imp_path.pCost < 0)
			return false;
	...
	while (!idVector.empty()){
		...
		for(int gID:gSet){
			...
			if(p.pCost > 0){
				...
				path temp_path = solTemp.paths[gID];
...
Solution:: path Solution::RemoveDepotFromPath(path p, int node_id){
	...
	//aresta para controlar possível loop cado a aresta anterior já tenha sido modificada
	...
	while(it_edge != edges.end()){
		...
		if(isDefinitelyLessThan(fuel_available_node_b, 0.0) && it_edge->node_b != node_id){
			...
			//se não for linha de cobertura verificar a possíbilidade de inserir depot
			...
			if(!IsCLine(it_edge->node_a, it_edge->node_b)){
				...
				else{//se for diferente atualizar para a nova aresta
			...
			else{//se for linha de cobertura verificar a possibilidade de abastecer antes de
				...
				else{//se for diferente atualizar para a última aresta do trecho, que será
...
map<int,vector<int>> Solution::eval_solution(Solution::solution newSol){
	...
	for(pair<bool,solution> ps: paretoSet){
		...
		//almostEqual = isApproximatelyEqual(newSol.maxCost,psSol.maxCost,1.0);
...
void Solution::print_paretoSet(){
	for(pair<bool,solution> ps:paretoSet){
		solution sol = ps.second;
		cout << "depotNum: " << sol.depotsNum << " Maior custo: " << sol.maxCost << endl;
 	}
}
...
bool Solution::robotCapacity_val(path p){
	double fuel_required, robot_capacity;
	...
	for(edge e:p.edges){//percorrer todas as arestas
	...
	//check if the biggest walk (depot to depot) is lower than the robot capacity
...
vector<bool> Solution::paretoSetValidation(){
	vector<bool> val;
	for(pair<bool,solution> ps :paretoSet){
		solution s = ps.second;
		val.push_back(checkRestrictions(s));
	}
	return val;
}

//cl é um dos targets da linha de cobetura
Solution::path Solution::removeCL(path p, int cl){
	...
	auto edge_a = edges.end();
	auto edge_b = edges.end();
	auto edge_c = edges.end();

	//caso não tenha aresta, retornar
	if(p.edges.empty()){
		p.pCost = -1;
		return p;
	...
	auto it_edge = edges.begin();
	while(it_edge != edges.end()){
		//encontra a linha de cobertura
		if(it_edge->node_a == t_1 && it_edge->node_b == t_2){
			edge_b = it_edge;
			break;
		}
		else if(it_edge->node_b == t_1 && it_edge->node_a == t_2){
			edge_b = it_edge;
			break;
		}
		++it_edge;
	...
	//caso não seja possível posicionar o iterator
	//na aresta a ser removida, retornar
	if(edge_b == edges.end()){
		p.pCost = -1;
		return p;
	}

	//get pre (a) and pos(c) edges from b
	//it_edge = edges.begin();
	edge_a = it_edge;
	edge_c = it_edge;
	...
	//connect node on edge_a to node on edge_c
	edge_a->node_b = edge_c->node_b;

	//caso a linha de cobertura a ser removida seja parte de loop com node_a de edge_a
	//remover, pois essa conexão irá fazer um loop em edge_a
	if(edge_a->node_a == edge_a->node_b){

		//remover a aresta LC de fuelontarget
		auto itmap_node = p.fuelOnTarget.find(edge_b->node_a);
		if(itmap_node != p.fuelOnTarget.end())
			p.fuelOnTarget.erase(itmap_node);

		itmap_node = p.fuelOnTarget.find(edge_b->node_b);
		if(itmap_node != p.fuelOnTarget.end())
			p.fuelOnTarget.erase(itmap_node);
		...
		//decrease the targetNum (t1 and t2) removed
		p.targetsNum = p.fuelOnTarget.size();

		p.edges.clear();
		p.edges.insert(p.edges.begin(),edges.begin(),edges.end());

		//check if the robot capacity is validated;
		if(!robotCapacity_val(p)){
			//cout << "Problema removeCL"  << endl;
			p.pCost = -1;
		}
	...
	//verificar a capacidade do robô antes em node_a
	auto it_fuel = p.fuelOnTarget.find(edge_a->node_a);
	if(it_fuel != p.fuelOnTarget.end())
		fuel_on_node_a = it_fuel->second;
	else
		fuel_on_node_a = robot_capacity;

	fuel_on_node_b = fuel_on_node_a - edge_a->time;

	it_fuel = p.fuelOnTarget.find(edge_a->node_b);
	if(it_fuel != p.fuelOnTarget.end())
	...
	/*Precisamos reajustar os combustíveis disponível em todos os target após node_b
	 * até o próximo depot.
	 * */

	//partindo de edge_a adequar a quantidade de combustível nos demais nós
	it_edge = edge_a;
	//obter a próxima aresta
	++it_edge;
	//o node_b da aresta anterior é o node_a da aresta posterior
	fuel_on_node_a = fuel_on_node_b;
	while(it_edge != edges.end()){

		itmap_node = p.fuelOnTarget.find(it_edge->node_b);
		if(itmap_node != p.fuelOnTarget.end()){
			fuel_on_node_b = fuel_on_node_a - it_edge->time;
			it_edge->time = fuel_on_node_b;

			//o combustível no node_a da próxima aresta
			fuel_on_node_a = fuel_on_node_b;
			it_edge++;//testar a próxima aresta se node_b for target
		}
		else
			break;
	}
...
//return true if the path is circuit.
...
Solution::path Solution::Link_Sets_Node_out_in(path p, int out, int in){
	...
	while(it_edges != p.edges.end()){
		if(it_edges->node_a == baseID && it_edges->node_b == n_b){
			...
			if(it_depot != p.depots.end()){ //se for depot
				it_second = ++it_edges;
				break;
			}
		}
...
pair<Solution::path, string> Solution::bestInsertionCL(path p, int cl){
	...
	for(int i = 0; i < nCL;i++){
		...
		/*if(!IsCircuit(pathTemp))
			cout <<"circuito bestInsertionCL" <<endl;
...
Solution::path  Solution::GetPathFromNodeAToNodeB(path p, int node_a, int node_b){
	...
	for(edge e:p.edges){
		...
		if(found_node_a){
			...
			else if(e.node_a != input.getRobotBaseId(path_temp.robotID))//se for depot
...
void Solution::SortMultiplesAdjacentOnDepots(path* p){
	...
	while(!stack_dfs.empty()){
		//obter o elemento do top
		u = stack_dfs.top();

		//encontrar o nó u no map de adjacência
		it_map = map_adj.find(u);

		//se o nó tiver adjacente
		if(it_map!=map_adj.end()){

			//obter a posição do próximo adjacente a ser visitado
			prox = get<2>(it_map->second);

			//obter o par adjacente e posição da lista
			adj = get<0>(it_map->second).begin();

			//avançar o iterator para a posição do adjacente
			advance(adj,prox);

			//se não for o fim, ou seja tiver adjacentes  para visitar
			if(adj!= get<0>(it_map->second).end()){

				//apontar para o próximo adjacente
				get<2>(it_map->second)++;

				//obter o nó adjacente
				v = adj->first;

				// além do nó v, verificar a quantidade total de adjacentes que o nó u possui.
				size = get<0>(it_map->second).size();

				//se u possuir mais adjacente ou se u for a base
				if(size > 1 || u == baseID){
					//se a lista de branches não possui o nó u
					if(u!=branches_opened.top())
						branches_opened.push(u);

					//obter a posição do adjacente na lista de adjacências
					pos = adj->second;

					//inicializar e inserir um branch para o adjacente de u
					it_branches_list = branches_list.emplace_hint(it_branches_list,u,make_pair(0,list<pair<int,list<int>>>()));

					//inserir qual é a posição do adjacente na lista de adjacências no cabeçalho do branch
					//identificador para diferenciar entre os branches que um nó pode ter.
					it_branches_list->second.first = pos;
				}

				// se a lista de branches não for vazia
				if(!branches_list.empty()){
					branch_id = it_branches_list->second.first; // obter a posição de inserção do branch

					if(it_branches_list->second.second.size()  == branch_id){// se o branch não foi inicializado
						it_branches_list->second.second.emplace_back(pair<int,list<int>>());//incializa o branch vazio
					}


					//obtém um iterator para um branch
					auto branch_it = it_branches_list->second.second.begin();

					//localiza a lista de inserção, avançando o iterator para a posição do branch
					advance(branch_it,branch_id);

					//caso u seja a base na primeira passada
					if(u == baseID && u!=v){
						branch_it->second.emplace_back(u);//inserir a base no branch
					...
					// inserir o nó no branch
					branch_it->second.emplace_back(v);
					branch_it->first = it_branches_list->second.second.size();

				}

				//encontrar a referência de adjacente para o nó no map
				it_map = map_adj.find(v);

				//se o nó tiver adjacentes
				if(it_map!=map_adj.end()){
					//se não foi visitado, marcar como visitado
					if(get<1>(it_map->second) == 1){
						//inserir na pilha
						get<1>(it_map->second) = 2;

						stack_dfs.push(v);
					}
				}
			}//se não houver mais adjacentes para visitar
			else{
				it_map = map_adj.find(u);
				if(it_map!=map_adj.end()){
	...
	branch_order.emplace(baseID,order); //inseria a base como último nó de retorno
	...
	while(it_branches_list != branches_list.end() && branches_list.size() > branch_order.size()){
		...
		while(it_list != it_branches_list->second.second.end()){//enquanto não percorrer todos os branches do tronco
			...
			if(it_list->second.back() == exit_branch->first ){//se o nó de saída da lista é igual ao nó de saída procurado
		...
		if(it_branches_list == branches_list.end()){//se a lista de troncos chegou ao final, ou seja percorreu todas os branches procurando o branch de saída.
	...
	while(it_edges != p.edges.end() && it_edges->node_b != base_id){
		...
		if(it_target != p.fuelOnTarget.end()){
			...
			if(it_target == p.fuelOnTarget.end()){
				...
				while(it_target == p.fuelOnTarget.end()){//enquanto b não for target
					depots_ids.emplace_back(it_edges->node_b);
					++it_edges;
...
vector<int> Solution:: SPT_A_Star(map<int,vector<pair<int,double>>>list_adj, int start, int goal,int robot_id){
	...
	while(!pq.empty()){
		...
		if(it_path == path_info.end()){ //se o nó não está no caminho
			...
			while(it_adj != it_list->second.end()){//para cada adjacente
				...
				if(it_path == path_info.end()){//caso não esteja
				 //obter o custo do caminho do start até node
					...
					if(cost_node_2_goal < 1){//caso o posto seja o goal
						if(node == goal)
							robot_fuel = 0;
						else
...
void Solution::OpenRandomDepotsOnTargetsPath(path* p){
	...
	while(!ltargets.empty()){
		...
		while(it_edges!= ledges.end()){
			if(!IsCLine(it_edges->node_a,it_edges->node_b)){
				if(it_edges->node_a == target ){//se sair do target, deviar para o posto antes de sair
					...
					while(it_depot == p->depots.end() && it_edge_adjfuel != ledges.end()){//enquanto não for depot
		...
		while(!tls.empty()){
			...
			if(v_inter.size() > 0){
				...
				while(it_edge != l_edges.end()){
					...
					else if(it_edge->node_b == node_b){//se encontrar o fim do trecho sair;
						it_edge++;
						s_edge2cut = it_edge;
						break;
					}
				...
				while(it_edge != l_edges.end()){
					...
					if(fuel_remaining  < 0){
						...
						//inserir o depot caso não esteja na lista
							...
							if(it_depot == p->depots.end() && it_edge->node_b != input.getRobotBaseId(p->robotID))
...
int Solution::GetNearOpenedDepot(path p, int node_id){
	...
	if(it_fuel != p.fuelOnTarget.end()){//se for target
		robot_fuel_available = it_fuel->second;//obter a quantidade disponível
	}
	else{
		if(it_depot != p.depots.end()){//se o nó for um posto, o robô parte com capacidade máxima
			robot_fuel_available = input.getRobotFuel(p.robotID);
		}
		else{//se o depot for um nó for um depot não presente na lista de depot.
			return -1;
		}
	}
...
//adicionar node_a e node_b no grafo, caso node_a ou node_b sejam targets
//se node_a for target ligar a todos os depots do grafo
//se node_b for target ligar ao depot mais próximo. O objetivo e garantir que o robô tenha a maior capacidade
//de combustível possível ao atingir o target node_b.
map<int,vector<pair<int,double>>>  Solution::AddNodesOnGraph(map<int,vector<pair<int,double>>> list_adj,
	...
	if(it_fuel != p.fuelOnTarget.end() || node_a == base_id){//se node_a for target ligar a todos os depots respeitando a capacidade do robô
	...
	if(it_depot_b!= p.depots.end()){//caso haja o depósito associado ao target
		//obter a lista n2
	...
	else{//caso contrário ligar todos os depots ao target
