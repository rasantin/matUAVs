...
#include <optional>
	...
	private:
		...
		std::pair<int, int> getCLDirection(const path& p, int cl);
		...
		path GetPathFromBaseToNewCL(const path& p, int pos);
		path GetPathFromNodeAToNodeB(path p, int node_a, int node_b);
		path GetPathFromNewCLToBase(path p, int pos);
		path GetPathFromEdgeToNextDepot(const path& p, edge e);
		...
		edge GetNextCL(const path& p, int pos);

		int GetNearOpenedDepot(path p, int node_id);
		path GetSPTOverOpenDepots(const path& p, int node_start, int node_goal);
